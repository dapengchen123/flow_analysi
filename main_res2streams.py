...
from reid.models import ResNetflow2stream
...
def get_data(dataset_name, split_id, data_dir, batch_size, workers,
    ...
    train_processor = Preprocessor(train_set, root=[dataset.images_dir, dataset.flows_dir],
    ...
    val_loader = DataLoader(
        Preprocessor(dataset.val, root= [dataset.images_dir, dataset.flows_dir],
                     transform=transforms.Compose([
    ...
    test_loader = DataLoader(
        Preprocessor(list(set(dataset.query) | set(dataset.gallery)),
                     root= [dataset.images_dir, dataset.flows_dir],
                     transform=transforms.Compose([
                         transforms.RectScale(256, 128),
                         transforms.ToTensor(),
                         normalizer,
...
def main(args):
    ...
    if args.loss == 'xentropy':
        model = ResNetflow2stream(args.depth1, args.depth2, pretrained=False,
                       num_classes=num_classes,
                       num_features=args.features, dropout=args.dropout)
    elif args.loss == 'oim':
        model = ResNetflow2stream(args.depth1,  args.depth2, pretrained=True, num_features=args.features,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', type=str, default='cuhk03',
                        choices=['cuhk03', 'market1501', 'viper', 'dukemtmc', 'ilidsvid', 'ilidsvidflow'])
    ...
    parser.add_argument('--depth2', type=int, default=18,
                        choices=[18, 34, 50, 101, 152])
